...
class coal_npp:
            ...
            for index, row in df.iterrows():
                for column in df.columns:
                    if isinstance(row[column], str) and (
                        ...
                        column].__contains__('Region-wise') or row[column].__contains__('Grand') or row[
                                column].startswith('As per') or row[column].startswith('Pithead') or row[
    ...
    def merge_xlsx_file(self):
        ...
        merged_df['PLF%/Tentative PLF current month upto date'] = merged_df[
            'PLF%/Tentative PLF current month upto date'].astype(str)
