...
class plant_plf:
    def __init__(self):
        self.file_directory = r'C:\GNA\Data\Plant PLF\PLF Monthly Report'
        self.output_path = r'C:\GNA\Data\Plant PLF\Monthly Report Edited'
    ...
    def convert_xls_to_xlsx(self, file_path):
        ...
        for file in xls_files:
            self.output_path = os.path.join(file_path, os.path.splitext(os.path.basename(file))[0] + '.xlsx')
            ...
            try:
                ...
                workbook.close()
                app.quit()
                print(f"Conversion completed. File saved at: {self.output_path}")
    ...
    def plant_plf_report_monthly(self):
        ...
        while base_date > end_date:
            ...
            if response.status_code == 200:
                with open(f'{self.file_directory}/{start_date_2}.xls', 'wb') as f:
    ...
    def edit_xlsx_files(self):
        ...
        for xlsx_file in xlsx_files:
            output_file = os.path.join(self.file_directory, os.path.splitext(os.path.basename(xlsx_file))[0] + '.xlsx')
            # final_output_file = os.path.join(self.output_path, os.path.splitext(os.path.basename(xlsx_file))[0] + '.xlsx')
    ...
    def merge_xlsx_files_plant_plf(self):
        ...
        merged_file_path = os.path.join(self.final_directory, 'plant_plf.xlsx')
        merged_df.to_excel(merged_file_path, index=False)
        print(f"Merged file saved to '{merged_file_path}'")

    def get_data(self):
        plant_plf.plant_plf_report_monthly(self)
        plant_plf.convert_xls_to_xlsx(self, self.file_directory)
